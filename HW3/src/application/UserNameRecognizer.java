	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// MODIFIED: The first character must be alphabetic only (not numeric)
			...
			case 1: 
				...
				// MODIFIED: Now supports three special characters (., -, _)
				...
				// MODIFIED: Added support for minus (-) and underscore (_) in addition to period (.)
				// . - _ -> State 2
			...
			case 2: 
				...
				// The next character MUST be alphanumeric to ensure special characters are between
			...
			if (running) {
				...
				else finalState = false;  // State 2 is not a final state
		...
		case 0:
			...
			// MODIFIED: Updated error message to reflect alphabetic-only requirement
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				// MODIFIED: Updated error message to include all allowed special characters
				userNameRecognizerErrorMessage += 
					"A UserName may only contain letters (A-Z, a-z), digits (0-9), and special characters (. - _).\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			...
			// MODIFIED: Updated error message to reflect all special characters
			userNameRecognizerErrorMessage +=
				"A character after a special character (. - _) must be alphanumeric (A-Z, a-z, 0-9).\n" +
				"Special characters cannot be consecutive or at the end of the username.\n";
			return userNameRecognizerErrorMessage;
